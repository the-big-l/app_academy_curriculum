...
def ford_supporting_films
  ...
  execute(<<-SQL)
    select m.title
    from movies m
    join castings c1 on m.id = c1.movie_id
    -- join castings c2 on c1.actor_id = c2.actor_id
    join actors a on a.id = c1.actor_id
    where a.name = 'Harrison Ford' and c1.ord != 1
  SQL
end
...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    FROM
      movies m
    JOIN
      castings c1 ON c1.movie_id = m.id
    JOIN
      castings c2 ON c1.movie_id = c2.movie_id
    JOIN
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    where a1.name = 'Art Garfunkel' and a2.name != 'Art Garfunkel'
  SQL
end
