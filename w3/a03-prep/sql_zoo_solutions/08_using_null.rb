...
def null_dept
  ...
  execute(<<-SQL)
    select name from teachers where dept_id is null
  SQL
end
...
def all_teachers_join
  ...
  execute(<<-SQL)
    ...
    from teachers t
    left outer join depts d on d.id = t.dept_id
  SQL
end
...
def all_depts_join
  ...
  execute(<<-SQL)
    select t.name, d.name
    from teachers t
    right outer join depts d on d.id = t.dept_id
  SQL
end
...
def teachers_and_depts
  ...
  execute(<<-SQL)
    ...
    from teachers t
    left outer join depts d on t.dept_id = d.id
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    ...
    from depts d
    left outer join teachers t on t.dept_id = d.id
    group by d.name
  SQL
end
